      ...
      visitor: {
        Program(programPath) {
          ...
          if (shouldImport) {
            ...
            const specifiers = imports.map(specifier => t.ImportSpecifier(t.identifier(specifier), t.identifier(specifier)));
  
            if (specifiers.length !== 0 && shouldImport) {
              programPath.node.body.unshift(t.ImportDeclaration(specifiers, t.StringLiteral(importPathLibrary)));
            }
          }
        },
              ...
              "FunctionExpression|ArrowFunctionExpression"(fnPath) {
                ...
                function recurBlock(current, yields) {
                  if (yields.length === 0) {
                    ...
                    if (yieldPath.node.declarations[0].init.arguments.length !== 1) {
                        throw Error('Yield(...) should just have one parameter')
                    }
                ...
                //try {
                    ...
                    forPath.replaceWith(returnStatement.argument);
                /*} catch(e) {
                    throw Error('Yield CallExpression should be used in VariableDeclaration inside a For CallExpression')
                }*/
              }
            });
          }
